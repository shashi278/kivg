...
class ShapeAnimator:
    ...
    @staticmethod
    def setup_animation(
        caller: Any, context: AnimationContext
    ) -> Optional[List[Animation]]:
        """
        ...
        for path in closed_shapes[shape_id][shape_id + "paths"]:
            ...
            for element in path:
                if isinstance(element, Line):
                    lp = line_points(
                        element, [*widget.size], [*widget.pos], [*sw_size], sf
                    )
                    path_elements.append([(lp[0], lp[1]), (lp[2], lp[3])])
    ...
    @staticmethod
    def _calculate_base_point(
        path_data: List[List[Tuple[float, float]]], direction: Optional[str]
    ) -> Optional[float]:
        """
    ...
    @staticmethod
    def _setup_line_animation(
        widget: Any,
        shape_id: str,
        line_count: int,
        line_points: List[Tuple[float, float]],
        base_point: Optional[float],
        ...
        """
        is_horizontal = (
            direction in ("left", "right", "center_x") if direction else False
        )
