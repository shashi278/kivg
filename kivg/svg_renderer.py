...
class SvgRenderer:
    ...
    def update_canvas(
        widget: Any,
        path_elements: List[Union[Line, CubicBezier]],
    ...
    @staticmethod
    def collect_shape_points(
        tmp_elements_lists: List[List[Any]], widget: Any, shape_id: str
    ) -> List[float]:
        """
            ...
            for element in path_elements:
                ...
                if len(element) == 2:  # Line (start, end)
                    shape_list.extend(
                        [
                            getattr(
                                widget, f"{shape_id}_mesh_line{line_count}_start_x"
                            ),
                            getattr(
                                widget, f"{shape_id}_mesh_line{line_count}_start_y"
                            ),
                    ...
                    shape_list.extend(
                        get_all_points(
                            (
                                getattr(
                                    widget,
                                    f"{shape_id}_mesh_bezier{bezier_count}_start_x",
                                ),
                                getattr(
                                    widget,
                                    f"{shape_id}_mesh_bezier{bezier_count}_start_y",
                                ),
                            ),
                            (
                                getattr(
                                    widget,
                                    f"{shape_id}_mesh_bezier{bezier_count}_control1_x",
                                ),
                                getattr(
                                    widget,
                                    f"{shape_id}_mesh_bezier{bezier_count}_control1_y",
                                ),
                            ),
                            (
                                getattr(
                                    widget,
                                    f"{shape_id}_mesh_bezier{bezier_count}_control2_x",
                                ),
                                getattr(
                                    widget,
                                    f"{shape_id}_mesh_bezier{bezier_count}_control2_y",
                                ),
                            ),
                            (
                                getattr(
                                    widget,
                                    f"{shape_id}_mesh_bezier{bezier_count}_end_x",
                                ),
                                getattr(
                                    widget,
                                    f"{shape_id}_mesh_bezier{bezier_count}_end_y",
                                ),
                            ),
                        )
                    )
